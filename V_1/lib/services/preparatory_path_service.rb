class PreparatoryPathService
  ...
  def calc_path_and_possible(longest_sequence, customers, distance_matrix)
    ...
    longest_sequence.each_with_index do |vertex, index|
      puts "#" * 90
      puts "longest_sequence = #{longest_sequence}; longest_sequence_length = #{longest_sequence_length}; vertex = #{vertex}; index = #{index}"
      if index != longest_sequence_length - 1
        customers.each do |customer|
          if vertex == customer.id - 1
            if current_time == 0 
            ...
            if !next_vertex.nil?
              ...
              if next_vertex[:to] > until_next_vertex
                puts "next_vertex[:to] = #{next_vertex[:to]}; until_next_vertex = #{until_next_vertex}"
                ...
                puts "delay_time = #{delay_time}; common_delay_time = #{common_delay_time}"
              else 
                way << vertex - 1 
                puts("Not possible")
